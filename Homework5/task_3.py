...
def everything_for_your_cat(cats_data):
    """
    Функция формирует словарь {people: cats}, после чего из словаря формирует исходящую строку и возвращает ее
    :param cats_data: входной кортеж
    :return: результат
        """
    owner_dic = {}
    our_str = ""
    for record in cats_data:
        owner_dic.setdefault(f"{record[2]} {record[3]}", []).append(f"{record[0]}, {record[1]}")
    for owner, cat in owner_dic.items():
        our_str += owner + ": " + "; ".join(cat) + "\n"
    return our_str
